...
export default function ChannelSummaryTable({ data }: Props) {
  ...
  const grouped = useMemo(() => {
    ...
    for (const row of data) {
      ...
      if (!map[key]) {
        map[key] = {
          item_type_name: row.item_type_name || '(unknown)',
          channel: row.channel,
  ...
  return (
    <div>
      <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100 mb-3">
        Channel Wise Summary <span className="text-primary-500">→</span>
      </h3>
