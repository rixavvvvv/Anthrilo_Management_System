...
type SortKey = keyof ItemWiseRow;
...
export default function ItemWiseReportTable({ data }: Props) {
  const [sortKey, setSortKey] = useState<SortKey>('item_type_name');
  const [sortDir, setSortDir] = useState<SortDir>('asc');
  const [page, setPage] = useState(1);
  const PAGE_SIZE = 50;

  const grouped = useMemo(() => {
    const map: Record<string, ItemWiseRow> = {};
    const seenSkus: Record<string, Set<string>> = {};
    for (const row of data) {
      const key = row.item_type_name || '(unknown)';
      if (!map[key]) {
        map[key] = {
          item_type_name: key,
      ...
      const sku = (row as any).item_sku_code || '';
  ...
  return (
    <div>
      <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100 mb-3">
        Item Wise <span className="text-primary-500">→</span>
      </h3>
      <div className="overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-700">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
            {paged.map((row) => (
              <tr key={row.item_type_name} className="hover:bg-slate-50/60 dark:hover:bg-slate-800/40 transition-colors">
