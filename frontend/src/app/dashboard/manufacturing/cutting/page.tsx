  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <button onClick={() => setCheckOpen(true)} className="btn btn-primary text-sm w-full sm:w-auto">Cutting Check</button>
      </div>
...
function CuttingOrderForm({ onSubmit, loading }: { onSubmit: (v: any) => void; loading: boolean }) {
  ...
  return (
    <form onSubmit={e => {
      ...
      <div className="flex justify-end pt-2"><button type="submit" disabled={loading || !form.garment_id || !form.fabric_id} className="btn btn-primary w-full sm:w-auto">{loading ? 'Creating…' : 'Create Cutting Order'}</button></div>
    </form>
  );
}
...
function CuttingCheckForm({ orders, onSubmit, loading }: { orders: CuttingOrder[]; onSubmit: (v: any) => void; loading: boolean }) {
  ...
  return (
    <form onSubmit={e => { e.preventDefault(); onSubmit(form); }} className="space-y-4">
      ...
      <div className="flex justify-end pt-2"><button type="submit" disabled={loading || !form.cutting_order_id} className="btn btn-primary w-full sm:w-auto">{loading ? 'Saving…' : 'Record Check'}</button></div>
    </form>
  );
}
